...
export default function QuoteForm() {
  ...
  // Calculate the total effective path
  const totalEffectivePath = items.reduce((sum, item) => {
    const pathLength = parseFloat(item.pathLengthArea) || 0;
    const passes = parseFloat(item.passes) || 0;
    const quantity = parseFloat(item.quantity) || 0;
    return sum + (pathLength * passes * quantity);
  }, 0);

  // Calculate the estimated cost
  const estimatedCost = totalEffectivePath * (parseFloat(form.rate) || 0);
      ...
      <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-4xl">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div className="col-span-2 md:col-span-1">
              <label className="block text-sm font-medium text-gray-700">No. of Passes</label>
              <div className="mt-1 flex rounded-md shadow-sm space-x-2">
                ...
                <select
                  value={form.passes > 4 ? form.passes : ''}
                  onChange={(e) => setForm(prev => ({ ...prev, passes: parseInt(e.target.value) }))}
                  className="flex-1 appearance-none px-4 py-2 rounded-lg bg-gray-200 text-gray-700 hover:bg-gray-300 transition-colors duration-200"
                >
                  <option value="" disabled>...</option>
                  {[...Array(10).keys()].map(i => i + 5).map(num => (
          ...
          <div className="mt-6 border-t pt-6">
            <div className="flex justify-between items-center mb-2">
              <span className="font-semibold text-gray-700">Total Effective Path</span>
              <span className="text-lg font-bold text-gray-900">
                {totalEffectivePath.toFixed(3)}
              </span>
            </div>
